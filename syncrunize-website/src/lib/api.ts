...
// Cache auth session in memory to avoid blocking localStorage fetch on EVERY request
...
// Export function to clear session cache (called on logout)
...
api.interceptors.request.use(async (config) => {
  ...
  try {
    // Check both cache age AND token expiration
    const now = Date.now();
...
// Enhanced response interceptor with performance monitoring and detailed error logging
api.interceptors.response.use(
  ...
  error => {
    ...
    if (!error.response) {
      return Promise.reject(new Error(`Network error - cannot reach backend at ${baseURL}. Please verify backend is running.`));
    }
