-- ========================================
-- PERFORMANCE OPTIMIZATION: Optimized Query Functions (FIXED)
-- ========================================
...
BEGIN
  ...
  IF NOT v_is_own_profile AND p_current_user_id IS NOT NULL THEN
    SELECT EXISTS(
      ...
      WHERE f.follower_id = p_current_user_id AND f.followed_id = p_target_user_id
    ) INTO v_is_following;
  ELSE
...
BEGIN
  ...
  LEFT JOIN (
    SELECT gl.group_post_id, COUNT(*) AS likes_count
    FROM group_likes gl
    GROUP BY gl.group_post_id
  ) lc ON gp.group_post_id = lc.group_post_id
  LEFT JOIN (
    SELECT gc.group_post_id, COUNT(*) AS comments_count
    FROM group_comments gc
    GROUP BY gc.group_post_id
